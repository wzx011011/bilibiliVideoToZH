...
PROMPT_TEMPLATE = (
    ...
    "【重要】这是一条普通的聊天消息,请在对话中直接输出处理后的完整朗读文本。\n"
    "禁止使用文档、画布、写作、代码块等任何生成工具,只输出纯对话文本。\n"
    "第一句话就必须是正文内容。禁止反问、禁止询问是否需要调整、禁止总结或缩短。\n"
    "禁止输出任何承诺、说明、确认、寒暄，例如'好的''我将''以下是''没问题''我来帮你'等。"
    "你的回复只能包含朗读文本本身,长度必须与原文相当。\n\n"
    "待处理的字幕文字：\n\n{text}"
)


def strip_boilerplate(text: str) -> str:
    """去掉豆包回复开头的客套话/承诺，只保留朗读正文。

    豆包有时会回复"好的，我将严格按照要求…"之类的前缀，需要截掉。
    """
    import re
    boilerplate_re = re.compile(
        r"^(好的|没问题|当然|我将|我会|我来|以下是|下面是|这是|"
        r"根据|按照|收到|明白|了解)[^。\n]*[。\n]",
        re.MULTILINE,
    )
    for _ in range(3):
        m = boilerplate_re.match(text.strip())
        if not m:
            break
        text = text.strip()[m.end():].strip()
    return text.strip()
