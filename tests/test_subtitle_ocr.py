...
# 持久化往返
# ---------------------------------------------------------------------
# 注：OCR 输出清洗曾用 _clean_ocr_output 处理 qwen3-vl 视觉模型的输出
# （<think> 块、引号、"图中字幕：" 前缀等）。该视觉模型路线已下线，
# 改用 RapidOCR（onnxruntime）后不再产生这类噪声，清洗逻辑及对应测试一并移除。
# RapidOCR 自身的中文行过滤逻辑见 subtitle_ocr._rapidocr_chinese_lines。
